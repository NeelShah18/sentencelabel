...
def base_text(text, flag):
	'''basae_text returns sentiment of text
...
def get_solid_setiment(__prob_sentiment):
	'''Convert floating point sentimrnt to 5 level of setiment between 0(very negative) to 4(very good)

	Args:
		__prob_sentiment(float): Setiment value of input.

	Returns:
		__Solid_sentiment(int): Return floating sentiment to integer criteria of sentiment.
	'''
	#set feault value of __solid_setiment
	__solid_setiment = 2
...
class get_label(object):
	'''get_label is class to get setiment of the given text with different options.

	Args:
		text(str): text to analysis it's sentiment.
		on_base(charter): Way to analysis text based on emoji through 'e' or text through 't'
		flag (boolean): True --> It gives 5 criteria 0,1,2,3,4 where 2(Nutral), 4(very positive), 1(very negative)
						False --> Gives probability with 2 floating point accuray between -1(negative) to 1(positive)
	Returns:
		result(dictonary): if their is not problem in text
							{
								'sentiment' : value(int or float)
							}
	...
	def get_label(self, text, on_base = "t", flag_prob=False):
		...
		if __on_base == "e":
			result = {"sentiment" : base_emoji(self.text, self.flag_prob)}
		elif __on_base == "t":
			result = {"sentiment": base_text(self.text, self.flag_prob)}
		else:
...
def test():
	...
	'''
	x = base_emoji("I love python 👨 :-)", True)
	print(x)
	return None
